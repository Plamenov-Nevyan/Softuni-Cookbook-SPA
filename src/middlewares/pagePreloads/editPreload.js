   ...
   let isInfoCorrect = validateInputs([name, img, ingredients, steps])
    ...
    if(isInfoCorrect){
        ...
        ctx.page.redirect(`/details/${ctx.params.recipeId}`)
          ...
          if(!isIngredientsCorrect && !isStepsCorrect){ctx.showNotification(`Ingredients and Steps must be atleast 3 in number!`, `errorBox`); return}
          else if(!isIngredientsCorrect){ctx.showNotification(`Ingredients must be atleast 3 in number!`, `errorBox`); return}
          else if(!isStepsCorrect){ctx.showNotification(`Steps must be atleast 3 in number!`, `errorBox`); return}
         }
    }
    else{
        ctx.showNotification(`Please fill all fields!`, `errorBox`); return
    }
}